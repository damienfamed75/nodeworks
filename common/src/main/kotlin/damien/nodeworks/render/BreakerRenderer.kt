...
import damien.nodeworks.client.BreakerEmissiveModel
 ...
 *    the User / Placer: 5 px long, 4×4 cross-section, in the throat region
 *    where the model is hollow enough for the beam to read.
 */
open class BreakerRenderer(context: BlockEntityRendererProvider.Context) :
    ...
    companion object {
        /** Length of the interior network-laser stub, matching the User /
         *  Placer. 5 px runs the beam from block centre (pixel 8) to pixel 3
         *  along the back-front axis, landing in the model's throat region
         *  where it's visible past the solid back body. */
     ...
     *  in breaker_emissive.png contribute zero (additive blend) and only
    ...
    /** Apply the same rotation the chunk renderer applies to the Breaker
     *  body model via `blockstates/breaker.json`'s variants - identical
     *  variant table to the User / Placer, so the math matches
