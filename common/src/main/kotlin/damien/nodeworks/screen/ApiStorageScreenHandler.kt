...
class ApiStorageScreenHandler(
...
) : AbstractContainerMenu(ModScreenHandlers.API_STORAGE, syncId) {
    ...
    companion object {
        ...
        fun clientFactory(syncId: Int, playerInventory: Inventory, openData: ApiStorageOpenData): ApiStorageScreenHandler {
            val dummy = SimpleContainer(ApiStorageBlockEntity.TOTAL_SLOTS)
            return ApiStorageScreenHandler(syncId, playerInventory, dummy, openData.pos)
        }

        fun createServer(syncId: Int, playerInventory: Inventory, entity: ApiStorageBlockEntity, pos: BlockPos): ApiStorageScreenHandler {
            ...
            return ApiStorageScreenHandler(syncId, playerInventory, entity, pos, data)
        }
    }
    ...
    fun isSlotActive(slotIndex: Int): Boolean {
        ...
        val activeCount = when (upgradeLevel) {
            0 -> ApiStorageBlockEntity.BASE_SLOTS
            1 -> ApiStorageBlockEntity.UPGRADE_1_SLOTS
            2 -> ApiStorageBlockEntity.UPGRADE_2_SLOTS
            3 -> ApiStorageBlockEntity.UPGRADE_3_SLOTS
            else -> ApiStorageBlockEntity.UPGRADE_4_SLOTS
        }
    ...
    private class ApiCardSlot(
        ...
        x: Int,
        y: Int,
        private val handler: ApiStorageScreenHandler
    ) : Slot(container, index, x, y) {
        override fun mayPlace(stack: ItemStack): Boolean {
            return stack.item is ProcessingApiCard && handler.isSlotActive(index)
        }
    }
